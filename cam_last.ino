...
static esp_err_t stream_handler(httpd_req_t *req){
  ...
  add_cors_headers(req); // Add CORS headers
...
void startCameraServer(){
  ...
  config.max_uri_handlers = 16; // Increased to accommodate OPTIONS handlers
  ...
  if (httpd_start(&httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(httpd, &options_index);
    httpd_register_uri_handler(httpd, &options_stream);
    httpd_register_uri_handler(httpd, &options_commands);
